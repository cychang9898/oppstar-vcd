...
const gyp = cb => {
  console.log('build');
  // https://stackoverflow.com/a/25254146/19336104
  // Calling node-gyp without npx will need to install node-gyp globally
  const proc = process.platform === 'win32' ? cp.spawn('cmd', ['/c', 'npx', 'node-gyp', 'configure', 'build']) : cp.spawn('npx', ['node-gyp', 'configure', 'build']);
